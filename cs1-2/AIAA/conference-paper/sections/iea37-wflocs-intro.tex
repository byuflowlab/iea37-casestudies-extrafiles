...
Complex computational methods such as DNS or LES use the Navier-Stokes equations, which require extensive computational time that is prohibitive in multi-iterative testing.
...
%Factors such as step size or the ``temperature'' tolerances in simulated annealing optimization methods can permit algorithms to escape local concavities in a design space in order to pursue a superior optimum.
...
our case studies are the first time an international collaboration has been conducted to comparatively and empirically analyze optimization methods and EWM selection on a representative WFLO problem.% to determine best practices when addressing the WFLO problem.
