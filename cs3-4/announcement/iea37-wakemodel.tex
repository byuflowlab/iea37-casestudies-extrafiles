...
\usepackage[colorlinks,bookmarks,bookmarksnumbered]{hyperref}
\hypersetup{
    colorlinks=true,
                ...
                \item For each wind speed bin as $V_{\infty}$, iterate through each turbine in the field to compute its power:
                    \begin{itemize}
                        ...
                        \item Compute effective wind speed ($V_{e}$) at each turbine:
                            ...
                            \end{equation*}
                        \item Use $V_{e}$ and the IEA37 10MW power curve to calculate each turbine's power:
                            \begin{equation}
                                \label{Eq:Power}
                                P_{turb}(V_{e}) = 
                                \begin{cases} 
                            ...
                            \end{equation}
                        \end{itemize}
                    \item Sum powers from all $t$ turbines for that wind speed
                        \begin{equation}
                            P_{speed} = \sum_{k=1}^t P_{turb, k}
                        \end{equation}
                    \item Sum powers for all $m$ wind speed bins for that wind direction, multiplied by the probability that each speed will occur
                        \begin{equation}
                            P_{dir} = \sum_{j=1}^m w_{j} P_{speed, j}
                        \end{equation}
            \end{enumerate}
        \item Compute AEP using farm power for all $n$ directions where $P$ is the wind farm power for direction $i$ and $f$ is the corresponding frequency for direction $i$.
        The factor of 8760 is to multiply by hours in a year:
        \begin{equation}
        AEP = \left(\sum_{i=1}^{n} f_i P_{dir, i}\right) 8760 \frac{\textrm{hrs}}{\textrm{yr}}
        \end{equation}
