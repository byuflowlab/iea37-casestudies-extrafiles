function [turb_coords, fname_turb, fname_wr] = getTurbLocYAML(file_name)
    ...
    coords = defs.position(1);
    turb_coords = zeros(length(coords.items),2);
    turb_coords(:,1) = cell2mat(coords.items(:,1));
    turb_coords(:,2) = cell2mat(coords.items(:,2));
   % Read Turbine locations into an array
   
   ref_list_turbs = defs.wind_plant(1).properties(1).turbine(1);
   ref_list_wr = defs.plant_energy(1).properties(1).wind_resource(1).properties(1);
