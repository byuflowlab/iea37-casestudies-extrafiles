...
This is the "main" function to run an optimization for IEA37's cs3 using:
    Boundry constraint: Normal Method, (boundary snipped)
...
if __name__ == "__main__":
    ...
    for i in range(100):
        if(path.exists('turblocs-prestarts-' + str(numRestarts) + 'run(' + str(i) + ').csv') == False):
            np.savetxt('turblocs-prestarts-' + str(numRestarts) + 'run(' + str(i) + ').csv', listTurbLocs, delimiter=',')
            break
    
    for i in range(100):
        if(path.exists('turblocs-prestarts-' + str(numRestarts) + 'run(' + str(i) + ')-time.csv') == False):
            np.savetxt('turblocs-prestarts-' + str(numRestarts) + 'run(' + str(i) + ')-time.csv', timeArray, delimiter=',')
            break
