...
if __name__ == "__main__":
    ...
    for i in range(100):
        if(path.exists('turblocs-bpm-' + str(numRestarts) + 'run(' + str(i) + ').csv') == False):
            np.savetxt('turblocs-bpm-' + str(numRestarts) + 'run(' + str(i) + ').csv', listTurbLocs, delimiter=',')
            break
    
    for i in range(100):
        if(path.exists('turblocs-bpm-' + str(numRestarts) + 'run(' + str(i) + ')-time.csv') == False):
