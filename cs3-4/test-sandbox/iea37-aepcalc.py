...
from scipy.integrate import quad        # For integration of probability
...
def calcWeibullInt(x, L, k):
    """Given a set of [Lambda, k] Weibull variables,
        returns the integral of the Weibull function <x>"""
    if x < 0:
        return 0
    else:
...
def binWindSpeeds(speed_weib, num_speed_bins, min_speed, max_speed):
    """Given an array of [Lambda, k] Weibull variables,
        creates <num_speed_bins> many probabilities and speeds"""
    bin_divs = np.linspace(min_speed, max_speed, num=(num_speed_bins+1))
    ...
    for i in range(num_dir_bins):
        for j in range(num_speed_bins):
            # The area under the curve is the diff of the integral evaluated at start and end of bin.
    ...
    wind_speeds = np.ones((num_dir_bins, num_speed_bins))*9.0
...
def calcAEPcs3(turb_coords, wind_freq, wind_speeds, wind_speed_probs, wind_dir,
    ...
    num_speed_bins = wind_speeds.shape[1]   # Number of wind speed bins

    # Power produced by the wind farm from each wind direction
    pwr_prod = np.zeros(num_dir_bins)
    ...
    for i in range(num_dir_bins):
        ...
        for j in range(num_speed_bins):
            ...
            pwr_prod_ws[i][j] = DirPower(turb_coords, wind_dir[i], wind_speeds[i][j],
                                        turb_diam, turb_ci, turb_co,
                                        rated_ws, rated_pwr) * wind_speed_probs[i][j]
        pwr_prod[i] = sum(pwr_prod_ws[i])

    #  Convert power to AEP
    hrs_per_year = 365.*24.
    AEP = hrs_per_year * (wind_freq * pwr_prod)
...
def getWindRoseYAML(file_name):
    ...
    wind_speed_coeffs = np.asarray(props['speed']['default'])
    # Get default number of windspeed bins per direction
    num_speed_bins = props['speed']['num_bins']
    min_speed = props['speed']['minimum']
    max_speed = props['speed']['maximum']

    return wind_dir, wind_freq, wind_speed_coeffs, num_speed_bins, min_speed, max_speed
...
if __name__ == "__main__":
    ...
    wind_dir, wind_freq, speed_weib, num_speed_bins, min_speed, max_speed = getWindRoseYAML(
        fname_wr)
    ...
    wind_speeds, wind_speed_probs = binWindSpeeds(speed_weib, 30, min_speed,
        max_speed)
    
    # Calculate the AEP from ripped values
    AEP = calcAEPcs3(turb_coords, wind_freq, wind_speeds, wind_speed_probs, wind_dir,
                turb_diam, turb_ci, turb_co, rated_ws, rated_pwr)
